...
import prisma, { SalesforceAccount } from '../../../lib/prismadb';
...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<{ users: SalesforceAccount[]; count: number }>
) {
  if (req.method !== 'GET') {
  ...
  const users = await prisma.salesforceAccount.findMany({
    where: { customerId },
