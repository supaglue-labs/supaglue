...
:::info
A customer is an end-user in your application. You can create a Supaglue customer through the management portal, or programatically when they signup in your application using the [Management API](/api/v2/mgmt/upsert-customer).
:::
...
This creates a Supaglue connection, which you can verify by checking the customers page in the management portal. Upon successful connection, CRM common object records will start syncing to your Postgres database.
...
## 9. Use the [Action API](api/introduction) to create a Contact
...
```
curl --location 'http://localhost:8080/crm/v2/contacts' \
--header 'x-customer-id: {CUSTOMER_ID}' \
--header 'x-provider-name: {PROVIDER_NAME}' \
--header 'x-api-key: {API_KEY}' \
--header 'Content-Type: application/json' \
