...
1. **Unified API**: Write to and read from multiple third-party providers with a single API within a category for [`crm`](v2/crm/unified-crm-api), [`engagement`](v2/engagement/unified-engagement-api), [`enrichment`](v2/enrichment/unified-enrichment-api), [`ticketing`](v2/ticketing/unified-ticketing-api-preview).
1. [**Metadata API**](v2/metadata/metadata-api): Operate on metadata across multiple third-party providers.
