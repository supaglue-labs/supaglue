...
Managed Syncs lets you offload dealing with third-party provider authentication, APIs, rate limits, retries, and more to Supaglue. We will sync data from third-party providers into a database that you can host or Supaglue can host.
...
3. Supaglue starts fetching data from your customers’ third-party Providers and landing them in a Destination (e.g. Postgres).
...
A Destination is a database that Supaglue syncs third-party provider data to. You can choose one in your [infrastructure](#option-1-your-database) a [Supaglue's infrastructure](#option-2-supaglue-hosted-database--data-listing-api).
...
### Table schema in Destination

The types of data that Supaglue writes in your Destination falls into three categories:
...
1. (**Normalized data**: The raw data is normalized: we hoist top-level raw data fields into table columns.)\*
...
Read more about [Objects](../platform/objects/overview) which will cover object names, table names, and table schemas.
...
Supaglue executes syncs on a per-[Object](../platform/objects/overview) basis. For example, Supaglue will create ten syncs if you sync ten Standard Objects. Supaglue schedules them to run simultaneously at the frequency you set. Each sync run is independent, i.e., a failure in one sync will not affect the others.
...
### Your database vs Supaglue-hosted database + [Data Listing API](../api/v2/data/data-listing-api)
