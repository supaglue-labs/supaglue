...
You can decide where the database that Supaglue syncs to is hosted:

1. **Self-host your database**: Supaglue writes raw data directly into your database.
1. **Use a Supaglue-hosted database + API**: Supaglue writes to a Supaglue-hosted database and provides an API for you to paginate, transform, and write data into your database schema.
...
Supaglue writes raw data into a Supaglue-hosted database and provides a convenient API to paginate over records. This is useful if you don't have an existing transformation pipeline.
...
### Your database vs Supaglue-hosted database + Data Listing API
