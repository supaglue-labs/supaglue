...
To obtain a `{CUSTOMER_ID}` for a customer, you can use the [Supaglue API](/api/crm) to create a new customer when they sign up for your application.

To obtain a `{PROVIDER_NAME}`, you can use the [Supaglue API](/api/crm) to create new integrations for your customers to have available for them to connect to.
