...
import type { Cursor, SupaglueRecord } from '.';
import { decodeCursor, getPaginatedSupaglueRecords } from '.';

describe('getPaginatedSupaglueRecords', () => {
  const TOTAL = 1000;
  const generateSampleRecords = (count: number, reverse = false, startId = 1): SupaglueRecord[] => {
    const results: SupaglueRecord[] = Array.from({ length: count }, (_, i) => ({
  ...
  test('should contain both next and previous pagination', () => {
    ...
    const result = getPaginatedSupaglueRecords(rows, TOTAL, 10, cursor);
  ...
  test('should contain only next pagination', () => {
    const rows = generateSampleRecords(11);
    const result = getPaginatedSupaglueRecords(rows, TOTAL, 10);
