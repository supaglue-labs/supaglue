...
class HubSpotClient extends AbstractCrmRemoteClient {
    ...
    return await paginator([
      {
        ...
        createStreamFromPage: (response) =>
          Readable.from(response.results.map((deal) => fromHubSpotDealToOpportunity(deal, pipelineStageMapping))),
        getNextCursorFromPage: (response) => response.paging?.next?.after,
      },
      {
        pageFetcher: archivedPageFetcher,
  ...
  private async getOpportunity(id: string, pipelineStageMapping?: PipelineStageMapping): Promise<Opportunity> {
